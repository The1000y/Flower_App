...
void main() {
  ...
  test(
    'should call getProducts on repository with occasionId and page and return SuccessResponce',
    () async {
      when(() => mockCommerceRepo.getOccasionsProducts(1, page: 1)).thenAnswer(
        (_) async => SuccessResponce<PaginatedProducts>(tPaginatedProducts),
      );
      ...
      expect(
        (result as SuccessResponce<PaginatedProducts>).data,
        tPaginatedProducts,
      );
  ...
  test('should propagate ErrorResponce when repository fails', () async {
    ...
    when(
      () => mockCommerceRepo.getOccasionsProducts(1, page: 1),
